...
#### Update 2021 
The original environment cannot run on the latest GPUs because the tensorflow version is too old and requires an old CuDNN version. Old CuCNN versions do not support new GPU models. 

I created a new environment and trained the models using a 2080 Ti GPU.
This environment is still compatible with the Udacity workspace. 

If you use a 3000 series or newer GPU, you have to install the latest libraries that are not backward compatible with tensorflow 1.x.x or Keras <=2.2.0. The syntax of many commands is different and you have to rewrite parts of the code.

```
conda create --name aind-vui python=3.6 numpy scipy tensorflow-gpu keras==2.2.0 seaborn jupyter ipykernel tornado==6.1  ipython ipykernel jupyter opencv
